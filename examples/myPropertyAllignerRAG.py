...
def dereference_properties(properties):
    ...
    for uri in properties:
        ...
        try:
            ...
            if any(rdf_type in content_type for rdf_type in ['xml', 'rdf', 'nt', 'trig', 'turtle']):
                g = rdflib.Graph()
                ...
                # If content is non-RDF, append the entire text content
                print(f"Warning: URI {uri} returned non-RDF content type: {content_type}. Adding full content to comparison.")
...
def calculate_semantic_similarity(properties1, properties2):
    ...
    based on the dereferenced content of the property URIs. It finds the best
    match for each property in properties1 from properties2.
    """
    ...
    for i, p1 in enumerate(properties1):
        # Find the index of the highest similarity score for the current property
        best_match_idx = np.argmax(cosine_sim[i])
        best_match_p2 = properties2[best_match_idx]
        best_score = cosine_sim[i, best_match_idx]
        
        results.append({
            "Property Source": p1,
            "Property Target": best_match_p2,
            "Semantic Similarity": best_score
        })
...
def main():
    ...
    try:
        ...
        print("### Property Alignment Results (TF-IDF with RAG-like approach, includes HTML)")
